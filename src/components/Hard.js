...
export const Hard = () => {
  ...
  const aiTurn = (currentBoxes) => {
    ...
    const emptyBoxes = currentBoxes.reduce(
      (acc, box, index) => (box === "" ? [...acc, index] : acc),
      []
    );

    const winningMoveIndex = checkForWinningMove(currentBoxes, "X");
    if (winningMoveIndex !== -1)
      return makeMove(currentBoxes, winningMoveIndex, "X");

    const blockingMoveIndex = checkForWinningMove(currentBoxes, "O");
    if (blockingMoveIndex !== -1)
      return makeMove(currentBoxes, blockingMoveIndex, "X");

    if (
      currentBoxes[4] === "O" &&
      [0, 2, 6, 8].some((index) => currentBoxes[index] === "X") &&
      currentBoxes[3] === "O" &&
      currentBoxes[5] === ""
    ) {
    ...
    if (
      currentBoxes[4] === "" &&
      emptyBoxes.length === 8 &&
      [0, 2, 6, 8].some((index) => currentBoxes[index] === "O")
    ) {
    ...
    if (
      ...
      const availablePositions = [1, 3, 5, 7].filter(
        (pos) => currentBoxes[pos] === ""
      );
      return makeMove(
        currentBoxes,
        availablePositions[
    ...
    const emptyCorners = emptyBoxes.filter((index) =>
    ...
    const emptySides = emptyBoxes.filter((index) =>
  ...
  const getOppositeCorner = (currentBoxes) => {
    ...
    const middle = 4;
    const userCornerIndex = currentBoxes.findIndex(
      (box, index) => corners.includes(index) && box === "O"
    );
  ...
  const checkForWinningMove = (boxes, symbol) => {
    for (const pattern of winPatterns) {
      const [a, b, c] = pattern;
      if (boxes[a] === symbol && boxes[b] === symbol && boxes[c] === "")
        return c;
      if (boxes[a] === symbol && boxes[b] === "" && boxes[c] === symbol)
        return b;
      if (boxes[a] === "" && boxes[b] === symbol && boxes[c] === symbol)
        return a;
    }
    return -1;
  };
  ...
  const checkWinner = (newBoxes) => {
    ...
    for (let pattern of winPatterns) {
      ...
      if (
        newBoxes[pos1] &&
        newBoxes[pos1] === newBoxes[pos2] &&
        newBoxes[pos1] === newBoxes[pos3]
      ) {
  ...
  const drawLine = (start, end) => {
    ...
    const containerRect = document
      .querySelector(`.${GameStyle.boxContainer}`)
  ...
  return (
    <div className={GameStyle.game} style={{ ...myStyle, boxShadow: "none" }}>
      ...
      <div className={GameStyle.mainBox}>
        ...
        <div
          className={`${GameStyle.msgContainer} ${msgVisible ? "" : "hide"}`}
        >
          <p id="msg">
            {winner === "DRAW" && "DRAW"}
